  ...
  useGSAP(() => {
    ...
    document.fonts.ready.then(() => {
      ...
      ctx = gsap.context(() => {
        ...
        // Speed: 100 pixels per second (faster)
    ...
    <div className="overflow-hidden py-1 md:py-2" ref={wrapperRef}>
      <div ref={containerRef} className="inline-block whitespace-nowrap will-change-transform">
        {tripleSkills.map((skill, i) => (
          <span key={i} className={`inline-block mx-3 md:mx-6 text-2xl md:text-6xl font-serif italic ${outline ? 'text-transparent stroke-black stroke-1 opacity-40' : 'text-black'}`}
          style={outline ? { WebkitTextStroke: '1px #151515' } : {}}>
            {skill} <span className="text-violet mx-2 md:mx-4 text-sm md:text-xl">•</span>
          </span>
        ))}
      </div>
    </div>
