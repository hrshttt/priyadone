...
const Navbar: React.FC = () => {
  ...
  // Logo Color Logic (Invert on Dark Sections)
  useGSAP(() => {
    const logo = logoRef.current;
    if (!logo) return;

    // Reset filter initially
    gsap.set(logo, { filter: "invert(0)" });
    ...
    darkSections.forEach((section) => {
      ScrollTrigger.create({
        trigger: section,
        start: "top 40px", // When section hits the navbar area
        end: "bottom 40px",
        onEnter: () =>
          gsap.to(logo, {
            filter: "invert(1)",
  ...
  // 1. Initial Entry Animation (Run once on mount)
  useGSAP(() => {
  ...
  // 2. Smart Scroll Logic (Refreshes on route change)
  useGSAP(() => {
    const el = navContainerRef.current;
    if (!el) return;

    // Refresh ScrollTrigger to ensure it understands the new page height
  ...
  // Black P Logo Data URI
  const BLACK_LOGO_SVG =
    "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Ctext x='50' y='75' font-family='Times New Roman, serif' font-style='italic' font-weight='bold' font-size='90' text-anchor='middle' fill='black'%3EP.%3C/text%3E%3C/svg%3E";
          ...
          <div className="hidden md:flex items-center gap-1 mr-2">
            {["About", "Work"].map((item) => {
              ...
              return (
                ...
                >
                  <span className="relative z-10 font-mono text-xs uppercase tracking-widest transition-colors duration-300 text-black">
                    {item}
                  </span>
                  {active && (
